...
%>
<h1><%=sub %> Examination marks</h1>
<form method="post" action="addexam.jsp?sub=<%=sub%>&year=<%=year%>" >
...
try
{
	...
	if(flag5==1)
	{
			response.sendRedirect("prac.jsp?sub="+sub+"&year="+year);
	}
	int no=2;
...
%>
<td>
<a href="editexam.jsp?&prn=<%=rs.getInt(1)%>">edit</a>
</td>
</tr>
<%
	}
...
<button><a href="prac.jsp?sub=<%=sub%>&year=<%=year%>" style="text-decoration:none">move to next page</a></button>>
